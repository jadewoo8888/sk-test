...
import framework.modules.propertymanagement.accountreceivablemanage.dao.WaAndWlecRevolvDAO;
import framework.modules.propertymanagement.accountreceivablemanage.domain.WaAndElecRevolv;
...
public class WaAndElecRevolvBO extends
	...
	public String retMoneyWaAndElecRevolv__log_trans(String pk, String retLister) {
		...
		WaAndElecRevolv revolv = getLastRevolv(client.getClientCode());
		// 第三部：保存周转金属性值
		if (revolv == null) {
			return_tips = "没有水电周转金！";
			return return_tips;
	 ...
	 * @return
	 */
	public WaAndElecRevolv getLastRevolv(String clientcode) {
		String lastRevolvSql = "";
		if (SystemConfig.getDBType() == DBType.SqlServer) {
			lastRevolvSql = "select top 1 * from tWaAndElecRevolv where clientcode=? order by lastestupdate desc";
		} else {
			lastRevolvSql = "select * from (select * from tWaAndElecRevolv where clientcode=? order by lastestupdate desc) where rownum=1";
		}
		WaAndElecRevolv revolv = entityDAO.executeFindEntity(
				WaAndElecRevolv.class, lastRevolvSql, clientcode);
		if (revolv == null) {
			return null;
		} else {
	...
	public Map<String, Object> getLastRegistAndCheckFlag(String clientcode) {
		...
		WaAndElecRevolv revolv = getLastRevolv(clientcode);
