...
function initComFunc() {
	...
	$("#id_div_fsss_title").click(fsssTitleClick);
 ...
 * 附属设施信息标题栏点击处理方法
 **/
function fsssTitleClick() {
	if (initedFsssFlag == false) {
		initedFsssFlag = true;
		initFsssDataGrid();
	}
	treatTitleIcon($("#id_div_fsss_titleIcon"));
	$("#id_div_fsss_gridarea").slideToggle("fast");
}
 ...
 * 初始化附属信息表格
 **/
function initFsssDataGrid() {
	var _sortInfo = {"sortPK":"arAccessoryPK", "sortSql":"lastestUpdate Desc"};
	var _columns = [[{field:"arAccessoryName", title:"\u8bbe\u65bd\u540d\u79f0", minwidth:270, align:"center", halign:"center"}, {field:"arAccessoryAssetValue", title:"\u539f\u503c\uff08\u5143 \uff09 ", minwidth:180, align:"center", halign:"center"}, {field:"arAccessoryPurchaseDate", title:"\u8d2d\u7f6e\u65e5\u671f", minwidth:140, align:"center", halign:"center"}]];
	var _dataGridOptions = {checkbox:false, height:250, width:900, pagination:false};
	var _customOptions = {tableID:"id_table_fsssgrid ", classID:"AssetRegistAccessoryBO", columns:_columns, sortInfo:_sortInfo, customQCFunc:setFsssDatagridQc};
	fsssDataGridObj = new DataGrid(_customOptions, _dataGridOptions);
}
/**
 * 初始化生命周期表格
 **/
