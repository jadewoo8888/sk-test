...
function initOpenWindow() {  
	openWindow = window.open("",  'listassetupload');
}
/**
 * 为页面上的组件添加事件处理方法
 **/
...
function save() {
	...
	if($("#id_filePath").val()!='') {
		...
		if(strControlType == STR_REGISTER_ADDNEW) {
			...
			uploadOrgCode = appendObj.appendEnpr;
		}
		var paramObj = {busitype:FILE_TYPE_ASSETAPPEND,progressid:progressID,orgcode:uploadOrgCode};
...
function uploadSuccesFunc(data) {
	if (data.status == "success") {  
		...
		if(strControlType == STR_REGISTER_ADDNEW) {
			appendRow.appendEnpr = top.strFilterOrgCode;
		}
		openWindow.addAppendRow(appendRow);
		changeBtnDisabled(false);
		$('#id_body_append').removeLoading();
...
function appendObjFactory() {
	...
	if(strControlType == STR_REGISTER_ADDNEW) { 
		...
		appendObj.assetRegAssetNo = strAssetRegAssetNo;
