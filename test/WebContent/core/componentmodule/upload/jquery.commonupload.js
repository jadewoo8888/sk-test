 ...
 * businessCode：业务单号,可能为空，当新增时 
 * businessType：业务类型
 **/
jQuery.fn.extend({commonupload:function (options) {
	...
	var op = jQuery.extend({controlType:STR_VIEW, businessCode:"", businessType:""}, options || {});
	/**
	 * 方法入口
	 **/
	function init() {
	...
	function initDataGrid() {
		...
		var customOptions = {tableID:"id_appendgrid_table", classID:"AppendBO", columns:_columns, sortInfo:_sortInfo, customQCFunc:setCustomQC};
	...
	function setCustomQC() {
		...
		qcArr[0].oper = 0;
		qcArr[0].value1 = op.businessCode;
		qcArr[1] = new Object();
		qcArr[1].fn = "businessType";
		qcArr[1].oper = 0;
		qcArr[1].value1 = op.businessType;
		return qcArr;
	}
	/**
	 * 附件列表双击处理方法
	...
	function rowDbClick(index, row) {
 	 ...
 	 **/
	function addAppend() {
		top.layer.open({type:2, title:"\u65b0\u589e\u4e0a\u4f20\u9644\u4ef6", shift:1, closeBtn:2, area:["440px", "220px"], content:contextPath + "/core/componentmodule/upload/editCommonUpload.jsp?controltype=" + STR_REGISTER_ADDNEW + "&openwindowname=" + window.name + "&areaid=" + _this.attr("id")});
	}
	/**
	 * 修改按钮点击处理方法
	 **/
	function modifyAppend() {
		...
		top.layer.open({type:2, title:"\u4fee\u6539\u4e0a\u4f20\u9644\u4ef6", shift:1, closeBtn:2, area:["440px", "220px"], content:contextPath + "/core/componentmodule/upload/editCommonUpload.jsp?controltype=" + STR_REGISTER_MODIFY + "&busitype=" + op.businessType + "&businesscode=" + op.businessCode + "&openwindowname=" + window.name + "&areaid=" + _this.attr("id")});
	}
	...
	function downLoad(row) {
		Ajax.download(FILE_TYPE_COMMONAPPEND, "info", row);
	}
