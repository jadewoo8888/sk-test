...
function addAppend() {
	top.layer.open({
		...
		closeBtn :2,
		area:['440px','220px'],
		content:contextPath+'/core/componentmodule/upload/editAssetUpload.jsp?controltype='+STR_REGISTER_ADDNEW+'&orgcode='+top.strFilterOrgCode
	});
}
...
function modifyAppend() {
	...
	top.layer.open({
		...
		closeBtn :2,
		area:['440px','220px'],
		content:contextPath+'/core/componentmodule/upload/editAssetUpload.jsp?controltype='+STR_REGISTER_MODIFY+'&assetregassetno='+strAssetRegAssetNo
	});
}
...
function setCustomQC(){ 
	...
	qcArr[0].oper = 0; 
	qcArr[0].value1 = strAssetRegAssetNo;
	return qcArr;
}
...
function downLoad(fileID) {
	...
	var data = appendDatagrid.dataGridObj.datagrid("getData").rows; 
 	...
 	Ajax.download(FILE_TYPE_ASSETAPPEND,'info',paramObj);
}
